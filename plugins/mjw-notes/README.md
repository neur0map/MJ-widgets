...
Flip card with a scrollable note pad and inline editing; notes autosave to the plugin's state.
...
- Reads: the plugin's notes.json
- Network: none
- Writes: the plugin's own state under
  `$XDG_STATE_HOME/ryoku/plugins/mjw-notes` (style choices and widget data only).
