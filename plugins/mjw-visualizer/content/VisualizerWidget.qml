...
WidgetBase {
    ...
    implicitWidth: isRing ? ringSize : Math.max(widthBudget > 0 ? widthBudget : 480, 320)
    ...
    component BarsVisualizer: Item {
        ...
        // Ryoku mounts the tile always, so at rest the bars keep a faint
        // sliver (upstream hid the widget entirely when silent).
        property real activityOpacity: 0.3
        ...
        Timer {
            id: silenceTimer
            interval: 1000
            onTriggered: bars.activityOpacity = 0.3
        }

        onPointsChanged: {
            if (points.some(p2 => p2 > 0)) {
