...
Live audio spectrum in five styles (bars, mirror, aurora, ring, dots) driven by your own playback stream.
...
- Reads: PipeWire playback monitor via cava (raw ascii frames)
- Network: none
- Writes: the plugin's own state under
  `$XDG_STATE_HOME/ryoku/plugins/mjw-visualizer` (style choices and widget data only).
  Nothing is written to shipped configs or other plugins' folders.
- Commands: cava (optional; the tile rests flat without it)
