...
Your photo masked into 34 Material shapes, drop-replaceable, sized from settings.
...
- Reads: the picked image file (read-only)
- Network: none
- Writes: the plugin's own state under
  `$XDG_STATE_HOME/ryoku/plugins/mjw-image` (style choices and widget data only).
