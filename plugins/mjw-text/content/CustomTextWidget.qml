...
WidgetBase {
    ...
    onEditingChanged: root._kb = root.editing
    Component.onDestruction: if (root.editing) root._kb = false
    ...
    // Background tap surface: the Ryoku host owns dragging, so a double click
    // on the tile body enters edit mode through this plain area instead of a
    // MouseArea root.
    MouseArea {
        anchors.fill: parent
        ...
        onDoubleClicked: (mouse) => {
            if (!root.editing) root.startEditing();
        }
    }
