...
Floating desktop text in eight bundled handwriting fonts (OFL), inline editing with Ctrl+Enter.
...
- Reads: the plugin's state entry
- Network: none
- Writes: the plugin's own state under
  `$XDG_STATE_HOME/ryoku/plugins/mjw-text` (style choices and widget data only).
