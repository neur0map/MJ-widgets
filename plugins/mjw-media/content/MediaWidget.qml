...
WidgetBase {
    ...
    function updateArt() {
        ...
        cacheProbe.target = root.artFilePath
    ...
    Process {
        ...
        command: target.length > 0 ? ["test", "-f", target] : ["true"]
        onExited: (code) => {
            if (code === 0) {
                root.downloaded = true
                return
            }
            coverArtDownloader.pendingUrl = root.artUrl
            root.downloaded = false
    ...
    Process {
        ...
        command: pendingUrl.length > 0
            ? ["curl", "-sSL", "--max-time", "8", pendingUrl, "-o", root.artFilePath]
            : ["true"]
