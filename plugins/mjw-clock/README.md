...
Analog cookie, digital and pixel-matrix clock faces in one tile, with hand, dial and date styles.
...
- Reads: clock time (Qt SystemClock)
- Network: none
- Writes: the plugin's own state under
  `$XDG_STATE_HOME/ryoku/plugins/mjw-clock` (style choices and widget data only).
